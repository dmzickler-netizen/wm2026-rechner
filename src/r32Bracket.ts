// AUTOMATISCH GENERIERT & PROGRAMMATISCH VALIDIERT — NICHT VON HAND EDITIEREN.
// QUELLE: FIFA World Cup 2026 Competition Regulations, Annex C (495 combinations); reproduced via Wikipedia '2026 FIFA World Cup knockout stage', fetched 2026-06-21
// Validierung: 495 eindeutige Konstellationen, alle C(12,8) abgedeckt,
// Mengengleichheit (zugewiesene Dritte == qualifizierte Dritte) und Slot-Erlaubnis je Match geprüft (0 Fehler).
// STATUS: VERIFIZIERT (interne Konsistenz). Vor Produktivnutzung gegen das offizielle FIFA-Regelwerk (Annex C) gegenprüfen.

export type GroupId = 'A'|'B'|'C'|'D'|'E'|'F'|'G'|'H'|'I'|'J'|'K'|'L';
export type SlotType = 'winner' | 'runnerUp' | 'third';

export interface R32Slot {
  type: SlotType;
  group?: GroupId;            // bei winner/runnerUp: feste Gruppe
  thirdAllowedGroups?: GroupId[]; // bei third: mögliche Herkunftsgruppen des Dritten
}
export interface R32Match { matchNo: number; home: R32Slot; away: R32Slot; }

// Die 16 Spiele der Runde der letzten 32 (Match 73–88).
export const R32_MATCHES: R32Match[] = [
  { matchNo: 73, home: { type: 'runnerUp', group: 'A' }, away: { type: 'runnerUp', group: 'B' } },
  { matchNo: 74, home: { type: 'winner', group: 'E' }, away: { type: 'third', thirdAllowedGroups: ['A','B','C','D','F'] } },
  { matchNo: 75, home: { type: 'winner', group: 'F' }, away: { type: 'runnerUp', group: 'C' } },
  { matchNo: 76, home: { type: 'winner', group: 'C' }, away: { type: 'runnerUp', group: 'F' } },
  { matchNo: 77, home: { type: 'winner', group: 'I' }, away: { type: 'third', thirdAllowedGroups: ['C','D','F','G','H'] } },
  { matchNo: 78, home: { type: 'runnerUp', group: 'E' }, away: { type: 'runnerUp', group: 'I' } },
  { matchNo: 79, home: { type: 'winner', group: 'A' }, away: { type: 'third', thirdAllowedGroups: ['C','E','F','H','I'] } },
  { matchNo: 80, home: { type: 'winner', group: 'L' }, away: { type: 'third', thirdAllowedGroups: ['E','H','I','J','K'] } },
  { matchNo: 81, home: { type: 'winner', group: 'D' }, away: { type: 'third', thirdAllowedGroups: ['B','E','F','I','J'] } },
  { matchNo: 82, home: { type: 'winner', group: 'G' }, away: { type: 'third', thirdAllowedGroups: ['A','E','H','I','J'] } },
  { matchNo: 83, home: { type: 'runnerUp', group: 'K' }, away: { type: 'runnerUp', group: 'L' } },
  { matchNo: 84, home: { type: 'winner', group: 'H' }, away: { type: 'runnerUp', group: 'J' } },
  { matchNo: 85, home: { type: 'winner', group: 'B' }, away: { type: 'third', thirdAllowedGroups: ['E','F','G','I','J'] } },
  { matchNo: 86, home: { type: 'winner', group: 'J' }, away: { type: 'runnerUp', group: 'H' } },
  { matchNo: 87, home: { type: 'winner', group: 'K' }, away: { type: 'third', thirdAllowedGroups: ['D','E','I','J','L'] } },
  { matchNo: 88, home: { type: 'runnerUp', group: 'D' }, away: { type: 'runnerUp', group: 'G' } },
];

// Map: Siegergruppe -> Match-Nr. des 'Sieger vs 3.'-Spiels
export const WINNER_TO_THIRD_MATCH: Record<string, number> = {"A": 79, "B": 85, "D": 81, "E": 74, "G": 82, "I": 77, "K": 87, "L": 80};

// Spaltenreihenfolge der Annex-C-Tabelle (Siegergruppen mit Drittplatzierten-Gegner)
export const WINNER_COLUMNS_ORDER = ["A", "B", "D", "E", "G", "I", "K", "L"] as const;

// Schlüssel = die 8 Herkunftsgruppen der qualifizierten Dritten, alphabetisch sortiert (z.B. 'EFGHIJKL').
// Wert = welche Drittplatzierten-GRUPPE der jeweilige Gruppensieger in der R32 trifft.
export type ThirdAssignment = Record<'1A'|'1B'|'1D'|'1E'|'1G'|'1I'|'1K'|'1L', GroupId>;
export const THIRD_PLACE_COMBINATIONS: Record<string, ThirdAssignment> = {
...
/**
 * Liefert die Drittplatzierten-Zuordnung für eine konkrete Achtergruppe.
 * @param qualifiedThirdGroups Array der 8 Gruppen, aus denen die besten Dritten kommen (z.B. ['A','B','C','D','E','F','G','H'])
 * @returns ThirdAssignment oder null, wenn die Konstellation nicht exakt 8 gültige Gruppen umfasst.
 */
export function getThirdPlaceAssignment(qualifiedThirdGroups: GroupId[]): ThirdAssignment | null {
  if (qualifiedThirdGroups.length !== 8) return null;
  const key = [...new Set(qualifiedThirdGroups)].sort().join('');
  return THIRD_PLACE_COMBINATIONS[key] ?? null;
}

/**
 * Baut die konkreten R32-Paarungen, sobald Gruppensieger, -zweite und die 8 Drittplatzierten-Gruppen feststehen.
 * teamOf: (slotLabel) => Teamname, z.B. teamOf('1A'), teamOf('2B'), teamOf('3E').
 */
export function resolveR32(
  qualifiedThirdGroups: GroupId[],
  teamOf: (label: string) => string,
): { matchNo: number; home: string; away: string }[] | null {
  const assign = getThirdPlaceAssignment(qualifiedThirdGroups);
  if (!assign) return null;
  const label = (s: R32Slot): string => {
    if (s.type === 'winner') return teamOf('1' + s.group);
    if (s.type === 'runnerUp') return teamOf('2' + s.group);
    // third: passende Siegergruppe dieses Matches finden
    const winnerGroup = (Object.keys(WINNER_TO_THIRD_MATCH) as GroupId[])
      .find((g) => WINNER_TO_THIRD_MATCH[g] === curMatchNo);
    const thirdGroup = winnerGroup ? assign[('1' + winnerGroup) as keyof ThirdAssignment] : undefined;
    return thirdGroup ? teamOf('3' + thirdGroup) : '3rd?';
  };
  let curMatchNo = 0;
  return R32_MATCHES.map((m) => {
    curMatchNo = m.matchNo;
    return { matchNo: m.matchNo, home: label(m.home), away: label(m.away) };
  });
}
